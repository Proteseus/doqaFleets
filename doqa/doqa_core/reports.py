...
def generate_fleet_overview_report():
    ...
    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/fleet_overview_report.pdf'
...
def generate_maintenance_summary_report():
    ...
    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/maintenance_summary_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []
...
def generate_alerts_analysis_report():
    # Analyze alert data
    total_alerts = Alert.objects.count()
    resolved_alerts = Alert.objects.filter(status='RESOLVED').count()
    unresolved_alerts = Alert.objects.filter(status='UNRESOLVED').count()
    unresolved_percentage = (unresolved_alerts / total_alerts) * 100 if total_alerts != 0 else 0

    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/alerts_analysis_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []

    data = [
        ['Alerts Analysis Report'],
        ['Total Alerts', total_alerts],
        ['Resolved Alerts', resolved_alerts],
        ['Unresolved Alerts', unresolved_alerts],
        ['Percentage of Unresolved Alerts', f'{unresolved_percentage:.2f}%' if total_alerts != 0 else 'N/A']
    ]
...
def generate_employee_utilization_report():
    ...
    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/employee_utilization_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []
...
def generate_trip_performance_report():
    ...
    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/trip_performance_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []
...
def generate_inventory_status_report():
    ...
    items_below_threshold = Inventory.objects.filter(quantity__lt=F('reorder_threshold')).count()
    restocking_frequency = Inventory.objects.aggregate(avg_restocking=models.Avg('last_restock_date'))

    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/inventory_status_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []
    ...
    data = [
        ...
        ['Items Below Threshold', items_below_threshold],
        ['Restocking Frequency (Days)', f'{restocking_frequency:.2f}' if restocking_frequency else 'N/A']
    ]
...
def generate_employee_vehicle_relationships_report():
    ...
    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/employee_vehicle_relationships_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []
...
def generate_system_health_report():
    ...
    total_vehicles = Vehicle.objects.count()
    active_vehicles = Vehicle.objects.filter(assigned=True).count()
    ...
    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/system_health_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []
...
def generate_fuel_efficiency_report():
    ...
    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/fuel_efficiency_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []
...
def generate_vehicle_maintenance_status_report():
    ...
    # Create PDF report
    filename = settings.MEDIA_ROOT + '/reports/vehicle_maintenance_status_report.pdf'
    doc = SimpleDocTemplate(filename, pagesize=letter)
    elements = []
...
def generate_and_merge_reports():
    ...
    reports = [
        ...
        generate_alerts_analysis_report(),
